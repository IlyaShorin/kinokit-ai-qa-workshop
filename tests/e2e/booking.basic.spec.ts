...
test('позволяет выбрать стандартные места и оформить бронирование', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Место A3, стандарт, 120 ₽, доступно' }).click();

  await expect(page.getByLabel('Выбранные места')).toHaveText('A2, A3');
  await expect(page.getByLabel('Количество выбранных мест')).toHaveText('Выбрано мест: 2');
  await expect(page.getByTestId('booking-total')).toHaveText('240 ₽');

  await page.getByRole('button', { name: 'Забронировать билеты' }).click();

  await expect(page.getByLabel('Подтверждение брони')).toHaveText('Бронь создана: A2, A3');
});
